...
def location():
    ...
    try:
        collectionArg = requestArgs.get('collection')

        if collectionArg is not "public" and collectionArg is not "staged":
            raise AttributeError()
    except AttributeError as err:
        return jsonify(
            'Invalid URI parameter "collection". \
             Values allowed: "public" and "staged"'), 400

    if collectionArg is "public":
...
def _GET(requestArgs, collection):
    # Enforce queryType
    try:
        queryType = requestArgs.get('queryType')

        if queryType is not "loc" and queryType is not "label":
            raise AttributeError()
    except AttributeError as err:
        return jsonify(
            'Invalid URI parameter "queryType". \
             Values allowed: "loc" and "label"'), 400
...
def _DELETE(data, collection):
    if data.get('latitude', None) is not None:
        db_response = collection.delete_one({'latitude': data['latitude']})
    ...
    if data.get('longitude', None) is not None:
        db_response = collection.delete_one({'longitude': data['longitude']})
        if db_response.deleted_count == 1:
            response = {'ok': True, 'message': 'record deleted'}
        else:
            response = {'ok': True, 'message': 'no record found'}
